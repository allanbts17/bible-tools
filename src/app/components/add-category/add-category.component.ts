...
export class AddCategoryComponent implements OnInit {
  ...
  async saveCategory() {
    if (this.validateCategory()) {
      if (this.newCat) {
        ...
        await this.storageService.editCategories(
          this.category,
          this.initialCategoryName
        );
      }
  ...
  validateCategory() {
    ...
    this.showUsedMsg =
      this.categoryList.find((cat) => cat.category == this.category.category) !=
        undefined && this.newCat;
    this.showUsedOnEdit =
      this.categoryList.find((cat) => {
        cat.category == this.category.category &&
          this.category.category != this.initialCategoryName;
      }) != undefined && !this.newCat;
