...
export class AddVerseModalComponent implements OnInit {
  ...
  changeVersesInput(text) {
    ...
    this.verseInputTimeout = setTimeout(() => {
      ...
      if (this.showInputMessage) return;
      ...
      this.verse.passage.reference =
        this.selectedChapter.reference + ':' + text;
  ...
  getPassage(passId, index) {
    let aux;
    this.apiService.getPassage(this.selectedBible.id, passId).subscribe(
      (data) => {
        ...
        if (!this.passageOutput.some((el) => el === 0)) this.buildOutputText();
      },
      (error) => {
        if (error.status === 404) {
  ...
  async saveVerse() {
    let verseTopicName;
    if (this.newVerse) this.verse.date = moment().format('lll');
    ...
    if (this.showNewTopicInput) {
      ...
      if (this.selectTopicName != '') {
        let topic = this.topicList.find(
          (top) => top.name == this.selectTopicName
        );
        this.verse.topic = topic.id;
        verseTopicName = topic.name;
      }
    ...
    if (
      ...
      if (this.newVerse) this.addVerse(this.verse, verseTopicName);
      else this.editVerse(this.verse, verseTopicName);
      this.resetValues();
      this.modal.dismiss();
    } else {
      if (!this.validateVerse()) this.showRedText = true;
    }
  }
