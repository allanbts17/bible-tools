...
export class CustomAlertComponent implements OnInit {
  ...
  async deleteTopicAlert(top: Topic) {
    ...
    const alert = await this.alertController.create({
      ...
      message:
        '¿Que desea hacer con los versículos del tema: "' + top.name + '"?',
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel',
  ...
  async verseDeleteConfirmationAlert(verse: Verse) {
    ...
    const alert = await this.alertController.create({
      ...
      header: 'Confirmación',
      message:
        'Está seguro que desea eliminar ' + verse.passage.reference + '?',
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel',
  ...
  categoryHaveNotes(cat) {
    return (
      this.notes[cat.category].findIndex((note) => note.category == cat.id) !=
      -1
    );
  }
  ...
  topicHaveVerses(top) {
    return (
      this.verses[top.name].findIndex((verse) => verse.topic == top.id) != -1
    );
  }
  ...
  async moveNotesAlert(cat) {
    ...
    delete newCategories[
      newCategories.findIndex((category) => category.id == cat.id)
    ];
