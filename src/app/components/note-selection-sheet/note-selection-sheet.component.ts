...
export class NoteSelectionSheetComponent implements OnInit {
    ...
    this.verses.forEach((verse) => {
      const verseSegments = Array.from(
        document.querySelectorAll(
          '[data-verse-id="' + this.chapter.id + '.' + verse + '"]'
        )
      );
  ...
  async shareData() {
    ...
    if (canShare.value) {
      ...
      this.presentToast(
        'La acción de compartir no es compatible con este dispositivo.'
      );
    }
  }
  ...
  quiteUnseletedColorClass() {
    if (this.selectedMarkerColor != undefined) {
      let unselectedMarked = Array.from(
        document.getElementsByClassName(this.selectedMarkerColor)
      );
  ...
  updatePassageReference(data) {
    ...
    this.passageReference =
      this.chapter.reference + ':' + verseRanges.toString();
  ...
  highlightText(
    color: 'yellow' | 'orange' | 'red' | 'green' | 'blue' | 'none'
  ) {
