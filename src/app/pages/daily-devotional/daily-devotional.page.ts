...
export class DailyDevotionalPage implements OnInit {
  ...
  onScroll(e) {
    setTimeout(async () => {
      ...
      if (this.filterOn && this.searchTerm == '')
        empty = await this.storageService.filterNotesByParam(
          this.filterType,
          this.searchTerm,
          this.selectedTab
        );
      else empty = await this.storageService.loadMoreNotes(tab); //this.getFilteredNotes(tab)?.slice(pagSize*this.notePages[tab],pagSize*(this.notePages[tab]+1))
      e.target.complete();
      if (empty) {
        log('empty');
  ...
  filterTypeSelect(e) {
    ...
    if (this.searchTerm != '') {
      ...
      this.storageService.filterNotesByParam(
        value,
        this.searchTerm,
        this.selectedTab
      );
  ...
  enableScrolls(all = true) {
    if (all) {
      ...
      this.tabs.find((tab) => tab.name == this.selectedTab.name).disabled =
        false;
    }
  }

  searchbarFocus() {
    if (this.filterType == 'date') this.showDate = true;
  }
  ...
  presentNoteModal(note: Note = null) {
    if (note == null) this.addNoteModal.setToNewFunction(this.selectedTab);
    else this.addNoteModal.setToEditFunction(note);
  ...
  presentCategoryModal(category: Category = null) {
    if (category == null) this.addCategoryModal.setToNewFunction();
    else this.addCategoryModal.setToEditFunction(category);
  ...
  async loadCategories(categories?) {
    ...
    this.categoryList = categories
      ? categories
      : await this.storageService.getCategories();
    this.fillTabs();
    setTimeout(() => {
      this.updateSlides();
    }, 200);
  ...
  fillTabs() {
    ...
    if (this.categoryList !== null)
      this.categoryList.forEach((cat) =>
        this.tabs.push({ name: cat.category, id: cat.id, disabled: false })
      );
