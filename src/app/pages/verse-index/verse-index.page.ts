...
export class VerseIndexPage implements OnInit {
  ...
  getPassage(bible, passId, index) {
    let aux;
    this.apiService.getPassage(bible.id, passId).subscribe(
      (data) => {
        aux = data;
  ...
  async editVerse(data: Verse) {
    let topicName = this.topicList.find(
      (top: Topic) => top.id == data.topic
    ).name;
  ...
  presentTopicModal(topic: Topic = null) {
    ...
    if (topic == null) this.addTopicModal.setToNewFunction();
    else this.addTopicModal.setToEditFunction(topic);
  ...
  toogleFilter() {
    ...
    if (!this.filterOn) this.quiteFilter();
  }
  ...
  filterByCustomType(tabFilteredList) {
    ...
    filtered = tabFilteredList.filter((verse) => {
      var found =
        verse.text.toUpperCase().includes(this.searchTerm.toUpperCase()) ||
  ...
  getFilteredVerses(tab) {
    if (this.filterOn) return this.filterByCustomType(this.verses[tab]);
    else return this.verses[tab.name];
  }
  ...
  addScrollListener() {
    ...
    let myInterval = setInterval(() => {
      let scrollSlides = document.getElementsByClassName(
        'scrolled-slide-verses'
      );
  ...
  scrollToTop() {
    let scrollSlide = document.getElementById(
      `slide-${this.selectedTab}-verses`
    );
